...
const agentModules = {
  "orchestrator": handler_orchestrator_0,
};
const workflowModules = {
  "sample-answer": workflow_sample_answer_0,
};
...
const agentIdentities = {
  "orchestrator": { bindingName: "FLUE_ORCHESTRATOR_AGENT", className: "FlueOrchestratorAgent" },
};
const workflowIdentities = {
  "sample-answer": { bindingName: "FLUE_SAMPLE_ANSWER_WORKFLOW", className: "FlueSampleAnswerWorkflow" },
};

const userCloudflare = {};
const reservedCloudflareExportNames = new Set(["FlueOrchestratorAgent","FlueSampleAnswerWorkflow","FlueRegistry"]);
...
const agentExtension0 = resolveCloudflareExtension(agentModules["orchestrator"], "orchestrator", 'Agent');
const FlueOrchestratorAgent = class FlueOrchestratorAgent extends agentExtension0.base(Agent) {
  constructor(ctx, env) {
    const prepared = cloudflareAgents.prepare({ storage: ctx.storage, className: "FlueOrchestratorAgent", agentName: "orchestrator" });
    super(ctx, env);
...
const WrappedFlueOrchestratorAgent = agentExtension0.wrap(FlueOrchestratorAgent);
export { WrappedFlueOrchestratorAgent as FlueOrchestratorAgent };
const workflowExtension0 = resolveCloudflareExtension(workflowModules["sample-answer"], "sample-answer", 'Workflow');
const FlueSampleAnswerWorkflow = class FlueSampleAnswerWorkflow extends workflowExtension0.base(Agent) {
  async onRequest(request) {
    return dispatchWorkflow(request, this, "sample-answer");
  }

  async onFiberRecovered(ctx) {
    if (ctx.name?.startsWith('flue:workflow:')) {
      return handleFlueWorkflowFiberRecovered(ctx, this, "sample-answer");
    }
...
const WrappedFlueSampleAnswerWorkflow = workflowExtension0.wrap(FlueSampleAnswerWorkflow);
export { WrappedFlueSampleAnswerWorkflow as FlueSampleAnswerWorkflow };
