# Agentic GTM Hackathon monorepo
...
The backend consists of Cloudflare Workers organized by responsibility (see [Worker naming](#1-create-a-new-cloudflare-worker) below). Add `worker-*`, `orm-*`, and `webhook-*` apps under `apps/` as needed.
...
1. Start dev servers from the repo root:
   ...
   make dev
   ```
2. Open the frontend dev server:
   - `http://localhost:5174`
